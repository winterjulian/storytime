...
export class DragDropService {
  ...
  public removeDropZone(connectedDropZone: string) {
    this.connectedDropZones.update(array => array.filter(element => element !== connectedDropZone));
  }
  ...
  private generateCommandObject(event: CdkDragDrop<Issue[]>): DropCommand {
    return {
      type: event.previousContainer === event.container ? "reorder" : "transfer",
  ...
  private executeCommand(command: DropCommand) {
    ...
    if (command.type === 'reorder') {
      ...
      transferArrayItem(sourceArray, targetArray, command.sourceIndex, command.targetIndex);
    }
  }
  ...
  redo(): boolean {
    if (this.currentHistoryIndex() >= this.commandHistory.length - 1) return false;
  ...
  private reverseCommand(command: DropCommand) {
    ...
    if (command.type === 'reorder') {
      ...
      transferArrayItem(targetArray!, sourceArray!, command.targetIndex, command.sourceIndex);
    }
  }
